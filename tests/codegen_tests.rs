...
#[test]
fn cargo_toml_defaults_to_adk_0_6_0() {
    let mut agents = HashMap::new();
    agents.insert("agent".to_string(), llm_agent("Test."));

    let toml = get_cargo_toml(&project("test", agents));
    assert!(toml.contains("adk-agent = \"0.6.0\""));
    assert!(toml.contains("adk-core = \"0.6.0\""));
    assert!(toml.contains("adk-model = { version = \"0.6.0\""));
    assert!(toml.contains("adk-tool = \"0.6.0\""));
    assert!(toml.contains("adk-graph = { version = \"0.6.0\""));
}
